...
def main():
    ...
    user_current_history_id = gi.histories.get_current_history()['id']
    ...
    for f in sequencing_files:
        ...
        fastq_file_in_galaxy = get_galaxy_item(sequencing_galaxy_library_contents, 'file', fastq_filename)
        if not fastq_file_in_galaxy:
            # upload file
            fastq_file_in_galaxy = gi.libraries.upload_from_galaxy_filesystem(
            ...
            sequencing_galaxy_library_contents = gi.libraries.show_library(library_id=sequencing_galaxy_library['id'], contents=True)
            log.debug('>>> gi.libraries.show_library: %s' % sequencing_galaxy_library_contents)
        log.debug('>>> fastq_file_in_galaxy: %s' % fastq_file_in_galaxy)
